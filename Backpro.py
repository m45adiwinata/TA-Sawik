...
def run(bobot, bobot2):
  ...
  for sawix in range(12):
      data = [[9.7, 14, 6.9, 10.3, 17.1, 8.4, 12.1, 17.9, 21.4, 17.9, 10, 4.3, 9.9],
          ...
          [10.8, 4.2, 21, 9, 10.2, 16.2, 10.8, 10.8, 7.6, 14.2, 15.6, 6.2, 12.9, 19.5]]
      dataset = data[sawix]
      nmin = min(dataset)
      nmax = max(dataset)
      computed_data = []
      ...
      prediction = [[8.4, 12.1, 17.9, 21.4, 17.9, 10, 4.3, 9.9, 19.3],
                    ...
                    [16.2, 10.8, 10.8, 7.6, 14.2, 15.6, 6.2, 12.9, 19.5, 2.9],]
...
def testing(bobot, bobot2):
  ...
  for sawix in range(12):
      ...
      min = minimum(prediction_dataset)
      max = maximum(prediction_dataset)
      computed_data = []
      for data in prediction_dataset:
          computed_data.append(normalize(data, min, max))
      test_data = slidingWindow(computed_data)
      predictions = []
      for i in range(len(test_data)):
          value = n.test(test_data[i][0: 8])
